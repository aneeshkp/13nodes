...
static void update_latency(app_data_t *data, time_t msecs)
{
    ...
    if (msecs > data->max_latency) data->max_latency = msecs;
...
static void display_latency(app_data_t *data)
{
    ...
    if (data->dump_csv) {
        ...
        printf("\n\nLatency:   (%lu msgs received",
               data->received_count);
        if (data->display_interval_sec && data->received_count > last_count)
            printf(", %lu msgs/sec)\n",
                   (data->received_count - last_count) / data->display_interval_sec);
        else
            printf(")\n");

        printf("  Average: %ld msec\n"
               "  Minimum: %ld msec\n"
               "  Maximum: %ld msec\n",
               (data->received_count)
                          ...
                          pn_event_type_t type)
{
    ...
    case PN_CONNECTION_INIT: {
        ...
        pn_terminus_set_address(pn_link_source(data->receiver), data->target);
        ...
        if (pn_delivery_readable(dlv) && !pn_delivery_partial(dlv)) {
            ...
            if (data->latency) {
                ...
                if (pn_delivery_pending(dlv) > data->buffer_len) {
                    ...
                    if (!data->decode_buffer) fatal("cannot allocate buffer");
                }

                // read in the raw data
                len = pn_link_recv(data->receiver, data->decode_buffer, data->buffer_len);
                if (len > 0) {
                    // decode it into a proton message
                    pn_message_clear(data->message);
                    if (PN_OK == pn_message_decode(data->message, data->decode_buffer,
                                                   len)) {
                        time_t _then = pn_message_get_creation_time(data->message);
                        if (_then && _now >= _then) {
            ...
            if (data->debug) fprintf(stdout, "Message received!\n");
            ++data->received_count;
...
static int parse_args(int argc, char *argv[], app_data_t *app)
{
    ...
    while((c = getopt(argc, argv, "a:c:t:i:p:luv")) != -1) {
        ...
        case 'i': app->display_interval_sec = atoi(optarg); break;
        case 'v': app->debug++; break;
        case 'l': app->latency = 1; break;
        case 'p': app->pre_fetch = atoi(optarg); break;
        case 'u': app->dump_csv = 1; break;
        default:
...
int main(int argc, char *argv[])
{
    ...
    if (!app_data->decode_buffer) fatal("Cannot allocate encode buffer");
